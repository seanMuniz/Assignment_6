...
function showSelectedArtist(artistID)
{
  ...
  showBackgroundImg(artistID); 

}
function showBackgroundImg(artistID){
  // let background = document.getElementById("artist-backgronud"); 
  for (var i = 0 ; i < artists.length ; i++){
    if (artists[i].id == artistID){
      document.body.style.backgroundImage = `url("${artists[i].backgroundImg}")`; 
    }
  }
}
...
function showSongForm(songID){
  var flag; 
  var showForm = document.getElementById("song-form"); 
